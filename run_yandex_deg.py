...
def run_benchmark():
    ...
    print(f"=== Starting VIBE Benchmark for DEG on {dataset_name} ===", flush=True)
    ...
    summary_records = []
    summary_file = Path("results") / dataset_name / "deg_summary.json"
    ...
    for idx, definition in enumerate(defs, 1):
        ...
        for pos, query_arguments in enumerate(definition.query_argument_groups, 1):
            print(f"  -> Query group [{pos}/{len(definition.query_argument_groups)}]: eps={query_arguments[0]}", flush=True)
            ...
            # Store standard VIBE HDF5 result
            ...
            rec = {
                ...
                "search_time_ms": float(descriptor.get("best_search_time", 0)) * 1000,
                "build_time_s": float(build_time),
                "index_size_kb": float(index_size),
            }
            summary_records.append(rec)
            
            # Flush incremental summary
            with open(summary_file, "w") as f:
                json.dump(summary_records, f, indent=2)

    total_time = time.time() - t_start_total
    print(f"\n=== Benchmark Complete in {total_time/60:.2f} minutes! ===", flush=True)
    print(f"Results stored in HDF5 format under: results/{dataset_name}/{count}/deg/")
    print(f"Summary JSON saved at: {summary_file}", flush=True)
